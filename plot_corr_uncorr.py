...
for dataset_name, dataset_cfg in DATASETS_CONFIG.items():
    ...
    # ==================================================================
    pimax_path = os.path.join(RESULTS_PIMAX_DIR, f"{dataset_name}_pimax_h_uncorrected.csv")
    ...
    for epsilon in epsilons:
        ...
        for model_name, acc_data in model_accuracies.items():
            ...
            if acc_data[epsilon][0] > max["model"]:
                max["model"] = acc_data[epsilon][0]
                max["model_name"] = model_name
        print(f"Max model: {max['model']} ({max['model_name']})")
        ...
        # Save plot
        plot_file = os.path.join(PLOT_OUTPUT_DIR, f"{dataset_name}_epsilon_{epsilon}_uncorrected.png")
